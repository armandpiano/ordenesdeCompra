...
<div class="store-selector">
    <form method="POST" action="?action=prepare-store-preview">
        ...
        <select name="clientCode" id="clientCode" required>
            <option value="">-- Selecciona cliente --</option>
            <?php foreach ($clients as $client): ?>
                <?php $clientValue = $client->id()->value(); ?>
                <?php $clientLabel = trim($clientValue) . ' – ' . $client->name(); ?>
                <option value="<?php echo htmlspecialchars($clientValue, ENT_QUOTES, 'UTF-8'); ?>" <?php echo (string) $lastClientCode === $clientValue ? 'selected' : ''; ?>>
    ...
    <?php if (empty($clients)): ?>
        <div class="alert alert-warning" style="margin-top: 0.5rem;">No se pudieron cargar clientes desde SAE.</div>
    <?php endif; ?>
</div>
